...
if ('serviceWorker' in navigator) {
  navigator.serviceWorker.register('./sw.js').then(() => navigator.serviceWorker.ready).then(() => { offlineReady = true; updateConnection(); }).catch(() => { inputStatus.textContent = 'Mode offline belum siap. Gunakan HTTPS atau localhost dan muat ulang.'; });
}
...
if (SpeechRecognition) {
  ...
  micBtn.addEventListener('click', () => {
    ...
    try { recognition.start(); recording = true; micBtn.setAttribute('aria-label', 'Hentikan pencarian suara'); micBtn.setAttribute('aria-pressed', 'true'); inputStatus.textContent = 'Mendengarkan…'; }
    catch { inputStatus.textContent = 'Mikrofon belum siap. Coba lagi atau ketik pertanyaan.'; }
  });
  recognition.addEventListener('result', event => { userInput.value = event.results[0][0].transcript; inputStatus.textContent = 'Periksa hasil suara, lalu kirim pertanyaan.'; userInput.focus(); });
  recognition.addEventListener('error', event => { inputStatus.textContent = event.error === 'not-allowed' ? 'Izin mikrofon ditolak. Kamu tetap bisa mengetik pertanyaan.' : 'Suara belum terbaca. Coba lagi atau ketik pertanyaan.'; });
  recognition.addEventListener('end', () => { recording = false; micBtn.setAttribute('aria-label', 'Mulai pencarian suara'); micBtn.setAttribute('aria-pressed', 'false'); if (inputStatus.textContent === 'Mendengarkan…') inputStatus.textContent = 'Tidak ada suara terbaca. Coba lagi atau ketik pertanyaan.'; });
}
